...
public sealed class FileLibraryService
{
    ...
    public const long MaximumMapImageSize =
        50 * 1024 * 1024;
    ...
    {
        var safeName =
            Path.GetFileName(originalName).Trim();
        ...
        try
        {
            _dbContext.FileAttachments.Add(
                attachment);
    ...
    private static string? DetectMapImageType(
        ReadOnlySpan<byte> data)
    {
        ...
        if (data.Length >= 12 &&
            data.Slice(4, 4)
                .SequenceEqual("ftyp"u8) &&
            (
                data.Slice(8, 4)
                    .SequenceEqual("avif"u8) ||
                data.Slice(8, 4)
                    .SequenceEqual("avis"u8)
            ))
        {
            return "image/avif";
        }
    ...
    public async Task<StoredFileContent?>
        OpenForPlayerAsync(
            Guid worldId,
            Guid fileId,
            CancellationToken cancellationToken)
    ...
    public async Task<StoredFileContent?>
        OpenMapImageAsync(
            ...
            CancellationToken cancellationToken)
    {
        var allowed = await _dbContext.WorldMaps
            .AsNoTracking()
            .AnyAsync(
                map =>
                    map.Id == mapId &&
                    ...
                    (
                        ...
                        map.ImageLayers.Any(layer =>
                            layer.FileAttachmentId ==
                            fileId &&
                            layer.IsVisible &&
                            (
                                !playerView ||
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (restoreFolderId is null)
        {
            return new(
                null,
                "RestoreDestinationNotFound");
        }

        var destinationIsValid =
